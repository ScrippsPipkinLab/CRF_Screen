...
def normInput(fileList):
    ...
    # Normalize tables
    file_ctrlTab = sltCtrlTabs(fileList)
    ctrl_reads_sum = [sum(list(file_ctrlTab.columns[x])) for x in range(1, len(file_ctrlTab.colnames))]
    ctrl_norm_factor = [1000000.0/x for x in ctrl_reads_sum]
    all_normcount_list = []
    for idx, tab in enumerate(tabList):
        ctrl_norm_factor_idx = ctrl_norm_factor[idx]
        norm_counts = [list(tab['count'])[x]*ctrl_norm_factor_idx for x in range(0,len(tab))]
        tabList[idx]["%s_ctrlNorm_count"%fileNameNoFormatList[idx]] = norm_counts
        all_normcount_list += norm_counts
    len(all_normcount_list)
    all_nb = fit_nbinom(np.array(all_normcount_list))

    for idx, tab in enumerate(tabList):
        norm_counts = list(tab.columns[3])
        nb_probs = [st.nbinom.cdf(x, all_nb['size'], all_nb['prob'])*100 for x in norm_counts]
        tabList[idx]["%s_ctrlNorm_nbProb"%fileNameNoFormatList[idx]] = nb_probs
    
    inputTab = tabList[0]
    inputTab_nbProb = list(inputTab.columns[4])
    outTabList = []
    for idx in range(1, len(tabList)):
        tabx = tabList[idx]
        tabx_nbProb = list(tabx.columns[4])
        newtabx = Table()
        newtabx['shRNA'] = tabx['shRNA']
        Prob_shift = [(x-y) for index, (x,y) in enumerate(zip(tabx_nbProb, inputTab_nbProb))]
        newtabx["%s_nbProbshift"%fileNameNoFormatList[idx]] = Prob_shift
        outTabList.append(newtabx)   
    outTab = outTabList[0]
    for idx in range(1, len(outTabList)):
        outTab = join(outTab, outTabList[idx], join_type = "outer", keys = "shRNA")
    return(outTab)
...
for key, tab in ref_dict.items():
    ...
    newtab = normInput(all_files_list)
