...
def normInput(fileList):
    ...
    for idx, tab in enumerate(tabList):
        ...
        adjustment = float(ctrl_norm_factor_idx)/sum(list(tab['count']))*1000000
        norm_pctg = [list(tab['count'])[x]*adjustment for x in range(0,len(tab))]
        tabList[idx]["%s_ctrlNorm_pctg"%fileNameNoFormatList[idx]] = norm_pctg
    
    inputTab = tabList[0]
    inputTab_nbProb = list(inputTab.columns[3])
    outTabList = []
    for idx in range(1, len(tabList)):
        tabx = tabList[idx]
        tabx_nbProb = list(tabx.columns[3])
        newtabx = Table()
        newtabx['shRNA'] = tabx['shRNA']
        Prob_shift = [(x-y) for index, (x,y) in enumerate(zip(tabx_nbProb, inputTab_nbProb))]
        newtabx["%s_adjPctgShift"%fileNameNoFormatList[idx]] = Prob_shift
...
wk_dir = "/Volumes/Yolanda/CRF_Screen/InVivo/1_1_Norm/20190124_3"
os.chdir(wk_dir)
